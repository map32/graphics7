...
void my_main( int polygons ) {
  ...
  for (i=0;i<lastop;i++) {  
	...
	break;
	case ROTATE:
	a = op[i].op.rotate.axis;
	switch (a){
		case 0:
		transform = make_rotX(op[i].op.rotate.degrees/180.*M_PI);
		break;
		case 1:
		transform = make_rotY(op[i].op.rotate.degrees/180.*M_PI);
		break;
		case 2:
		transform = make_rotZ(op[i].op.rotate.degrees/180.*M_PI);
		break;
	}	
	matrix_mult(s->data[s->top],transform);
	copy_matrix(transform,s->data[s->top]);
